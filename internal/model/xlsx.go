...
func CreateHostListInfoXlsx(Filedata []Hosts, HostType string) ([]byte, error) {
	...
	case "VM_WIN":
		...
		xlsx.SetColWidth("Sheet1", "E", "P", 20)
		//设置表头
		xlsx.SetCellValue("Sheet1", "A1", "编号")
		xlsx.SetCellStyle("Sheet1", "A1", "A1", stylecenter)
		xlsx.SetCellValue("Sheet1", "B1", "类型")
		...
		xlsx.SetCellValue("Sheet1", "C1", "名称")
		xlsx.SetCellStyle("Sheet1", "C1", "C1", stylecenter)
		xlsx.SetCellValue("Sheet1", "D1", "主机名")
		xlsx.SetCellStyle("Sheet1", "D1", "D1", stylecenter)
		xlsx.SetCellValue("Sheet1", "E1", "IP")
		xlsx.SetCellStyle("Sheet1", "E1", "E1", stylecenter)
		...
		xlsx.SetCellStyle("Sheet1", "J1", "J1", stylecenter)
		xlsx.SetCellValue("Sheet1", "K1", "位置")
		xlsx.SetCellStyle("Sheet1", "K1", "K1", stylecenter)
		xlsx.SetCellValue("Sheet1", "L1", "部门")
		xlsx.SetCellStyle("Sheet1", "L1", "L1", stylecenter)
		xlsx.SetCellValue("Sheet1", "M1", "资产编号")
		xlsx.SetCellStyle("Sheet1", "M1", "M1", stylecenter)
		xlsx.SetCellValue("Sheet1", "N1", "MAC地址")
		xlsx.SetCellStyle("Sheet1", "N1", "N1", stylecenter)
		xlsx.SetCellValue("Sheet1", "O1", "安装日期")
		xlsx.SetCellStyle("Sheet1", "O1", "O1", stylecenter)
		xlsx.SetCellValue("Sheet1", "P1", "维保到期时间")
		xlsx.SetCellStyle("Sheet1", "P1", "P1", stylecenter)
		xlsx.SetCellValue("Sheet1", "Q1", "备注")
		xlsx.SetCellStyle("Sheet1", "Q1", "Q1", stylecenter)
		//遍历数据
		for k, v := range Filedata {
			xlsx.SetCellValue("Sheet1", "A"+strconv.Itoa(k+2), v.HostID)
			xlsx.SetCellValue("Sheet1", "B"+strconv.Itoa(k+2), ValueTypeStr)
			...
			xlsx.SetCellValue("Sheet1", "K"+strconv.Itoa(k+2), v.Location)
			xlsx.SetCellValue("Sheet1", "L"+strconv.Itoa(k+2), v.Department)
			xlsx.SetCellValue("Sheet1", "M"+strconv.Itoa(k+2), v.ResourceID)
			xlsx.SetCellValue("Sheet1", "N"+strconv.Itoa(k+2), v.MAC)
			xlsx.SetCellValue("Sheet1", "O"+strconv.Itoa(k+2), v.DateHwInstall)
			xlsx.SetCellValue("Sheet1", "P"+strconv.Itoa(k+2), v.DateHwExpiry)
			xlsx.SetCellValue("Sheet1", "Q"+strconv.Itoa(k+2), v.Vendor)
		}
	case "VM_LIN":
	...
	default:
		// 自定义类型：按 is_hardware 字段选择硬件或虚拟机模板
		at, atErr := GetAssetTypeByCode(HostType)
		if atErr == nil {
			ValueTypeStr = at.Name
		} else {
			ValueTypeStr = HostType
		}
		if IsHardwareType(HostType) {
			// 硬件类型：使用硬件模板（同 HW_SRV）
			xlsx.SetColWidth("Sheet1", "B", "B", 20)
			xlsx.SetColWidth("Sheet1", "C", "C", 36)
			xlsx.SetColWidth("Sheet1", "D", "L", 20)
			xlsx.SetCellValue("Sheet1", "A1", "编号")
