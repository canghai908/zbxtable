...
func DatabaseInit() {
	...
	err = DB.Where("config_key = ?", "encryption_key").First(&encryptionKeyConfig).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		// 生成随机加密密钥
